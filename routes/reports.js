...
async function busOccupancyRows() {
  const buses = await db.prepare(`SELECT * FROM buses ORDER BY bus_number`).all();
  const out = [];
  for (const b of buses) {
    const occupied = (await db.prepare(
      `SELECT COUNT(*) AS c FROM students WHERE route_number = ? AND status='Active'`
    ).get(b.route_number)).c;
    out.push({
...
router.get('/bus-occupancy', async (req, res, next) => {
  try {
    res.json({ data: await busOccupancyRows() });
  } catch (err) { next(err); }
});
...
router.get('/bus-occupancy/export', async (req, res, next) => {
  try {
    ...
    ], await busOccupancyRows());
    sendXlsx(res, 'bus-occupancy.xlsx', buffer);
  } catch (err) { next(err); }
});
